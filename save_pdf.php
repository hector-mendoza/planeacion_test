...
$sql = "INSERT INTO materias VALUES('','$carr[carrera]','$unidad','$semestre', '$fecha', '$proposito', '$genericas', '$especificas',
...
for ($i=0; $i < $total_clases; $i++) { 
	...
	mysqli_query($link, "INSERT INTO unidadescompetencia VALUES ('', '$lastId', '$fechaUnidades', '$temaUnidades', '$ap_esp', '$ini$est$cierre', '$material', '$evid_ap', '$tipo_eval', '$inst_eval', '$criterio')");
}

//referencias
	foreach ($_POST['ref_bas'] as $refbas) {
		mysqli_query($link, "INSERT INTO referenciasxmateria VALUES('', '$lastId', '$refbas', 'BASICA') ");
		$ref_bas .= $refbas.",";
	}

	if ($_POST['ref_com']) {
		mysqli_query($link, "INSERT INTO referenciasxmateria VALUES('', '$lastId', '$_POST[ref_com]', 'COMPLEMENTARIA') ");
